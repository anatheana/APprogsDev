!! Provides input parser in Fortran 90/95
...
MODULE InputParser
  !! Double type FP_KIND from PaulV module
	USE Type_Kinds
  !! Hashtable for storing key/value pairs of input
	USE Hashtable
  !! PaulV string operations
	USE String_Utility
  !! PaulV file operations
	USE File_Utility
  !! My own wrapper for handling errors
	USE ErrorCapsule
  !! Some math- and printing routines
	USE AP_utils
	IMPLICIT NONE
  ...
  !! file_et_line-type integer input as the file name if read from input.
	...
	INTEGER :: i, j, k, my_err_id, astat
	REAL(FP_KIND) :: x, y, z
	LOGICAL :: b, is_init = .FALSE.
	CHARACTER :: c
	CHARACTER(LEN=max_msg_length) :: err_str
...
SUBROUTINE init_input(n)
  ...
  !! but not so efficiently.
	...
	CALL init_hash(n)
	my_err_id = get_err_id("SimanticsParser")
	is_init = .TRUE.
...
! PRIVATE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Checks is input is inited. Will stop the execution if not.
SUBROUTINE check_init()
	IMPLICIT NONE
	
	IF (is_init) RETURN
	
	WRITE(*,*) "Error from SimanticsParser: subroutine 'init_input'", &
...
SUBROUTINE setup_int(key, value_data)
	...
	IF(i > 0) THEN
		...
		CALL issue_error(my_err_id, inp_error_defined, err_str)				
		RETURN
	ELSE
...
SUBROUTINE setup_real(key, value_data)
	IMPLICIT NONE
	CHARACTER(LEN=*), INTENT(IN) :: key
	REAL(FP_KIND), INTENT(IN) :: value_data
...
SUBROUTINE get_int(key, value_data)
	...
	IF(i /= inp_type_int) THEN
		CALL issue_error(my_err_id, inp_error_wrong_type, "Wrong or nonexistent value when asked for int")
	ELSE
...
SUBROUTINE get_real(key, value_data)
	IMPLICIT NONE
	CHARACTER(LEN=*), INTENT(IN) :: key
	REAL(KIND=FP_KIND), INTENT(INOUT) :: value_data

	CALL check_init()
	
	i = hash_value_type(key)
	IF(i /= inp_type_real) THEN
			CALL issue_error(my_err_id, inp_error_wrong_type, "Wrong or nonexistent value when asked for real")
	ELSE
		i = get_hash_value(key,value_data)
	END IF
...
SUBROUTINE get_char(key, value_data)
	...
	IF(i /= inp_type_char) THEN
			CALL issue_error(my_err_id, inp_error_wrong_type, "Wrong or nonexistent value when asked for char")
	ELSE
		i = get_hash_value(key,value_data)
	END IF
...
SUBROUTINE get_logical(key, value_data)
	...
	IF(i /= inp_type_logical) THEN
		WRITE(str, *) "Wrong or nonexistent value when asked for logical for key '", TRIM(key), &
			"'. Status code was ", i
		CALL issue_error(my_err_id, inp_error_wrong_type, str)		
	ELSE
		i = get_hash_value(key,value_data)
	END IF
...
SUBROUTINE get_string(key, value_data, str_len)
	...
	IF(i /= inp_type_string) THEN
		CALL issue_error(my_err_id, inp_error_wrong_type, "Wrong or nonexistent value when asked for string")
	ELSE
...
SUBROUTINE read_input_fn(fn, DEBUG)
	IMPLICIT NONE
	...
	IF(astat /= 0) THEN
		...
		CALL issue_error(my_err_id, inp_error_file_open, err_str)
		RETURN
	END IF
	
	CALL read_input_ch(ch, DEBUG=db)
...
! INTERFACE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! For generic interface read_input
SUBROUTINE read_input_ch(ch, DEBUG, FN)
	IMPLICIT NONE
	...
	DO WHILE(astat == 0)
		...
		CASE DEFAULT
			...
			CALL issue_error(my_err_id, inp_error_wrong_type, err_str)
			RETURN
		END SELECT
		IF(astat /= 0) THEN
			WRITE(err_str, *) "Wrong variable type: ", TRIM(key)
...
!	EXTERNAL :: GETARG
	...
	DO ai=1,ac
		...
		IF(i == 0) THEN
			...
			CALL issue_error(my_err_id, inp_command_line, err_str)
			RETURN
		END IF
