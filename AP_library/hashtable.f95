!! Provides hashtable in Fortran 90/95
...
MODULE Hashtable
  ...
  !!  value_data is integer, value_data2 is string and str_len is its length.
	...
	INTEGER :: hash_table_length, astat, i, j, k, my_err_id
...
!! will issue error (see module ErrorCapsule).
SUBROUTINE init_hash(n)
  !! Input, max. number of different hash keys
	INTEGER, INTENT(IN) :: n

	my_err_id = get_err_id("Hashtable")
	
	hash_table_length = n
	ALLOCATE(hash_table(hash_table_length), STAT=astat)
	IF (astat /= 0) THEN
		CALL issue_error(my_err_id, error_allocation, "Error in allocating Hashtable.")
		RETURN
	END IF
...
! PRIVATE!!!!!!!!!!!!!!!!!!!!!!!!!!
! Test if hashtable is initialized
SUBROUTINE is_init()
	IMPLICIT NONE

	IF(.NOT. inited) THEN
		my_err_id = get_err_id("Hashtable")
		CALL issue_error(my_err_id, error_use_bef_allocation, &
			"Hashtable was tried to be used before allocation.")
		RETURN
	END IF
...
! PRIVATE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Add a new node to fist hash node
...
FUNCTION hash_index(key_str) RESULT(key)
	...
	WRITE(wkey_str,*) StrLowCase(key_str(1:key_length))
...
FUNCTION find_node(key, curr_node) RESULT(stat)
	...
	END IF
	! Check the key value
	DO WHILE(TRIM(curr_node%hash_key) /= TRIM(StrLowCase(key)))
...
FUNCTION delete_hash_node(key) RESULT(stat)
  ...
  !! result, status of operation
	...
	IF(ASSOCIATED(curr_node%next_node)) THEN
		DO WHILE(curr_node%hash_key /= StrLowCase(TRIM(key)))
			IF( .NOT. ASSOCIATED(curr_node%next_node)) THEN
				RETURN
			END IF
			prev_node => curr_node
...
FUNCTION add_hash_int(key, value_data, do_empty) RESULT(stat)
	...
	IF(LEN_TRIM(key) > hash_key_length) THEN
		new_node%hash_key = StrLowCase(key(1:hash_key_length))
	ELSE
		new_node%hash_key = StrLowCase(TRIM(key))
	END IF
...
FUNCTION add_hash_string(key, value_data, str_len, do_empty) RESULT(stat)
	...
	IF(str_len > max_string_length) THEN
		...
		CALL issue_error(my_err_id, error_string_too_long, err_msg)
		RETURN
	END IF
	IF(str_len > LEN(value_data)) THEN
		WRITE(err_msg,*) "String length ", str_len, " larger than the actual string"
		CALL issue_error(my_err_id, error_string_length_mismatch, err_msg)
		RETURN
	END IF
...
FUNCTION get_hash_real(key, value_data) RESULT(stat)
	IMPLICIT NONE
	CHARACTER(LEN=*), INTENT(IN) :: key
	REAL(FP_KIND), INTENT(INOUT) :: value_data
	INTEGER :: stat
	TYPE(hash_node), POINTER :: curr_node
