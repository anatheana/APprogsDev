...
MODULE Type_Kinds
  ...
  ! correspondence:
  ! -------------------------------------------------------------------
  !! Current chosen default integer
  !!    IFP     REAL( fp_kind )
  !!  ==============================
  !!     1       Single (4  bytes)
  !!     2       Double (8  bytes)
  !!     3       Quad   (16 bytes)  **IF AVAILABLE, Double OTHERWISE**
  INTEGER, PARAMETER, PRIVATE :: IFP = 2
  ...
  !! quad real
  INTEGER, PARAMETER, PUBLIC  :: Quad   = ( ( ( 1 + SIGN( 1, Quad_t ) ) / 2 ) * Quad_t ) + & ! Quad precision
  ...
  ! -- Default values
  !! Deafult real
  INTEGER, PARAMETER, PUBLIC  :: FP_Kind         = FP_KIND_TYPES( IFP ) ! Default value
  !! Deafult real bytes
  INTEGER, PARAMETER, PUBLIC  :: n_Bytes_FP_Kind = FP_BYTE_SIZES( IFP ) ! Default value
